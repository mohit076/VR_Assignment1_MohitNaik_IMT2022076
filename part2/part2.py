...
def process_images(input_folder, output_folder):
    ...
    for i in range(1, len(img_paths)):
        ...
        if result is None:
            continue
        left, viz = result
        cv2.imwrite(os.path.join(output_folder, f"{input_folder}_match{i}.jpg"), viz)

    cv2.imwrite(os.path.join(output_folder, f"{input_folder}_panorama.jpg"), left)
    print(f"Panorama and keypoint matches for {input_folder} created and saved in output folder.")
